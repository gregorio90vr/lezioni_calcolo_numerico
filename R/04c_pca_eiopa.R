...
#  Struttura e logica identiche a 04_pca_ecb.R: cambia la fonte dati.
...
#  Input : ../dati/04c_eiopa_spot.xlsx (prodotto da 04c_pca_eiopa_prep.R)
#  Output: file PDF in ../output/04c_pca_eiopa/ con suffisso _m
#  Il suffisso _m e' mantenuto per parallelismo con la lezione 04/04b; qui
...
#  come in 04_ecb_spot.xlsx. La conversione dai decimali degli archivi
#  EIOPA avviene una sola volta, in 04c_pca_eiopa_prep.R.
# ------------------------------------------------------------
...
input_xlsx <- "../dati/04c_eiopa_spot.xlsx"
input_ecb  <- "../dati/04_ecb_spot.xlsx"   # usato solo nella Sezione 5
output_dir <- "../output/04c_pca_eiopa"
...
if (!file.exists(input_xlsx)) {
  stop(paste0("File non trovato: ", input_xlsx,
              "\nEsegui prima 04c_pca_eiopa_prep.R oppure verifica il percorso."))
}
...
# Rispetto alla lezione 04, Lehman (2008), Draghi (2012) e il QE (2015)
...
# A differenza della lezione 04 (che usa il mese centrale), qui si usa il
...
#
# Esercizio SPECULARE a quello della lezione 04: là si proiettava una
...
if (!file.exists(input_ecb)) {
  stop(paste0("File non trovato: ", input_ecb,
              "\nServe la curva BCE della lezione 04 (esegui 04_pca_ecb_prep.R)."))
}

# --- Curva BCE AAA, ultima disponibile nel campione della lezione 04 ---
