...
# ------------------------------------------------------------
#  PCA sulle VARIAZIONI MENSILI della curva EIOPA RFR (EUR)
#  Struttura e logica identiche a 03_pca_ecb.R: cambia la fonte dati.
#  A differenza della curva BCE, quella EIOPA e' gia' mensile per
#  costruzione (una pubblicazione al mese, a fine mese): non c'e'
#  quindi alcun passo di aggregazione da giornaliero a mensile.
#  Input : ../dati/03c_eiopa_spot.xlsx (prodotto da 03c_pca_eiopa_prep.R)
#  Output: file PDF in ../output/03c_pca_eiopa/ con suffisso _m
#  Il suffisso _m e' mantenuto per parallelismo con la lezione 03/03b; qui
#  la frequenza mensile non e' una scelta dell'analista ma del regolatore.
#  UNITA': tutti i tassi sono in PUNTI PERCENTUALI (2.592 = 2.592%),
#  come in 03_ecb_spot.xlsx. La conversione dai decimali degli archivi
#  EIOPA avviene una sola volta, in 03c_pca_eiopa_prep.R.
# ------------------------------------------------------------
...
input_xlsx <- "../dati/03c_eiopa_spot.xlsx"
input_ecb  <- "../dati/03_ecb_spot.xlsx"   # usato solo nella Sezione 5
output_dir <- "../output/03c_pca_eiopa"
...
if (!file.exists(input_xlsx)) {
  stop(paste0("File non trovato: ", input_xlsx,
              "\nEsegui prima 03c_pca_eiopa_prep.R oppure verifica il percorso."))
}

# ==============================================================
# Lettura e pulizia (i dati EIOPA sono gia' mensili)
# ==============================================================

cat("\n=== STEP 1: LETTURA E PULIZIA (DATI GIA' MENSILI) ===\n")
...
# ------------------------------------------------------------------
# NESSUNA AGGREGAZIONE MENSILE
# EIOPA pubblica una sola curva al mese, riferita all'ultimo giorno di
# calendario: la matrice mensile coincide con i dati letti. Cade quindi
# anche la scelta -- necessaria per la BCE -- fra ultima osservazione e
# media mensile.
# ------------------------------------------------------------------

n_months      <- nrow(curve_df)
R_mat_m       <- as.matrix(curve_df[, ..maturity_cols])   # M x n
dates_m       <- curve_df$TIME_PERIOD                     # fine mese EIOPA
Delta_R_m     <- diff(R_mat_m)                            # (M-1) x n
dates_delta_m <- dates_m[-1]

cat("Curve mensili EIOPA caricate: ", n_months, " mesi (da ",
...
plot_curves_dt <- rbindlist(lapply(sel_idx, function(i) {
  data.table(Data    = format(dates_m[i], "%b %Y"),
...
p1 <- ggplot(plot_curves_dt, aes(x = MAT_NUM, y = Rate, color = Data, group = Data)) +
  ...
  labs(title = "Curve EIOPA in 3 mesi rappresentativi",
...
ggsave(file.path(output_dir, "step1_delta_mensili_m.pdf"),
...
# Eventi macroeconomici chiave del campione EIOPA (dic. 2015 - giu. 2026).
# Rispetto alla lezione 03, Lehman (2008), Draghi (2012) e il QE (2015)
# cadono prima dell'inizio del campione e sono stati sostituiti.
key_events <- data.table(
  date  = as.Date(c("2016-06-01", "2020-03-01", "2022-02-01",
                    "2023-09-01", "2024-06-01")),
  label = c("Brexit\n(giu. 2016)", "COVID + PEPP\n(mar. 2020)",
            "Svolta BCE\n(feb. 2022)", "Picco tassi\n(set. 2023)",
            "Primo taglio\n(giu. 2024)")
)
...
# ------------------------------------------------------------------
# Ricostruzione progressiva k = 0, 1, 2, 3
# A differenza della lezione 03 (che usa il mese centrale), qui si usa il
# mese di NORMA MASSIMA: il mese centrale del campione EIOPA cade in una
# fase piatta e poco istruttiva, mentre il mese di massimo movimento rende
# visibile il contributo di ciascuna componente.
# ------------------------------------------------------------------
ref_idx <- which.max(row_norms)
ref_ym  <- dates_delta_m[ref_idx]
cat("Mese di norma massima (ricostruzione progressiva):",
    format(ref_ym, "%B %Y"),
    " (norma =", round(row_norms[ref_idx] * 100, 1), "pb)\n")
...
p8 <- ggplot(prog_dt, aes(x = MAT_NUM, y = DeltaRate * 100,
  ...
  labs(title = "Ricostruzione progressiva della variazione mensile",
      subtitle = paste0("Mese di movimento massimo: ", format(ref_ym, "%B %Y")),
       x = "Scadenza (anni)", y = "Variazione mensile (punti base)",
...
# Confronto fit NS per 3 date (inizio, picco 10Y, recente)
...
writeLines(
  c(sprintf("Campione mensile analizzato: \\textbf{%d} mesi e \\textbf{%d} maturity.",
    ...
    sprintf("Varianza cumulata con $k=3$: \\textbf{%.2f\\%%}.",
            100 * cum_var[3]),
    ...
    sprintf("Mese di movimento massimo del campione: \\textbf{%s} (norma \\textbf{%.1f~pb}).",
            format(ref_ym, "%B %Y"), row_norms[ref_idx] * 100)
  ),
  summary_file
)
...
#
# Esercizio SPECULARE a quello della lezione 03: là si proiettava una
# curva EIOPA sui loadings BCE, qui si proietta una curva BCE sui
# loadings EIOPA. La base V non ha mai "visto" una curva BCE.
# ==============================================================

cat("\n=== SEZIONE 5: CALIBRAZIONE SU CURVA ARBITRARIA ===\n")

if (!file.exists(input_ecb)) {
  stop(paste0("File non trovato: ", input_ecb,
              "\nServe la curva BCE della lezione 03 (esegui 03_pca_ecb_prep.R)."))
}

# --- Curva BCE AAA, ultima disponibile nel campione della lezione 03 ---
# Gia' in punti percentuali: nessuna conversione necessaria.
ecb_dt <- as.data.table(read.xlsx(input_ecb, sheet = "PCA_Input"))
ecb_dt[, TIME_PERIOD := parse_time_period(TIME_PERIOD)]
setorder(ecb_dt, TIME_PERIOD)
ecb_last <- ecb_dt[nrow(ecb_dt)]

y_new <- as.numeric(ecb_last[, ..maturity_cols])
names(y_new) <- maturity_cols
y_date <- ecb_last$TIME_PERIOD

cat("Curva BCE usata:", format(y_date, "%d/%m/%Y"), "\n")
cat("Valori (%):\n")
print(round(y_new, 3))

# --- Base ortonormale dei loadings (da SVD sui dati mensili EIOPA) ---
V <- sv_m$v   # n x n, colonne ortonormali
n_mat <- ncol(V)

# --- Pesi e ricostruzioni per k = 3, 5, 10, n_mat ------------------
k_vals  <- unique(c(3L, 5L, 10L, n_mat))
...
# ==============================================================
# Grafico A — Curva originale vs k=3 e k=n_mat
# k=n_mat: base completa, ricostruzione esatta (curva sovrapposta)
# ==============================================================
lab_full <- paste0("k=", n_mat, " PC (esatta)")
df_levels <- rbindlist(list(
  data.table(Scadenza = maturities_num,
             Valore   = as.numeric(y_new),
             Tipo     = "Originale"),
  data.table(Scadenza = maturities_num,
             Valore   = recon_list[["k=3"]],
             Tipo     = "k=3 PC"),
  data.table(Scadenza = maturities_num,
             Valore   = recon_list[[paste0("k=", n_mat)]],
             Tipo     = lab_full)
))
df_levels[, Tipo := factor(Tipo, levels = c("Originale", "k=3 PC", lab_full))]
...
p_s5a <- ggplot(df_levels,
  ...
  scale_colour_manual(
    values = setNames(c("#000000", "#e66101", "#1a9850"),
                      c("Originale", "k=3 PC", lab_full))) +
  ...
  labs(
    ...
    subtitle = paste0("Curva BCE del ", format(y_date, "%d/%m/%Y"),
                      " proiettata sui loadings EIOPA"),
    x       = "Scadenza (anni)",
    y       = "Tasso (%)",
    colour  = NULL,
    linetype = NULL
...
# ==============================================================
# Grafico B — Convergenza su scala logaritmica (k = 1..n_mat-1)
...
# k=n_mat è escluso perché l'errore è rumore numerico.
# I tassi sono in punti percentuali: *100 converte in punti base.
# ==============================================================
err_norms_bp <- sapply(1:n_mat, function(k) {
  alpha_k <- as.numeric(t(V[, 1:k, drop = FALSE]) %*% y_new)
  y_hat_k <- as.numeric(V[, 1:k, drop = FALSE] %*% alpha_k)
  sqrt(sum((y_hat_k - y_new)^2)) * 100   # norma L2 in punti base
})

# Riduzione percentuale rispetto a k=1
riduzione_pct <- (1 - err_norms_bp / err_norms_bp[1]) * 100

cat(sprintf("\nErrore L2 (bp):  k=1: %.2f  |  k=3: %.2f  |  k=10: %.2f  |  k=%d: %.2e\n",
            err_norms_bp[1], err_norms_bp[3], err_norms_bp[10],
            n_mat, err_norms_bp[n_mat]))
...
# Escludi k=n_mat dal grafico log (errore ~0, rumore macchina)
df_err <- data.table(k = 1:(n_mat - 1L), Errore_bp = err_norms_bp[1:(n_mat - 1L)])
...
p_s5b <- ggplot(df_err, aes(x = k, y = Errore_bp)) +
  ...
  scale_x_continuous(breaks = 1:(n_mat - 1L)) +
  scale_y_log10(
...
# ==============================================================
# Grafico C — Errore di ricostruzione per singola scadenza (k=3 vs k=10)
# Il Grafico B mostra solo la norma L2 aggregata: qui si scompone
# l'errore tenor per tenor per capire DOVE si concentra il residuo.
# ==============================================================
delta_dt <- rbindlist(list(
  data.table(Scadenza = maturities_num,
             Delta_bp = (recon_list[["k=3"]]  - y_new) * 100,
             Tipo     = "k=3 PC"),
  data.table(Scadenza = maturities_num,
             Delta_bp = (recon_list[["k=10"]] - y_new) * 100,
             Tipo     = "k=10 PC")
))
delta_dt[, Tipo := factor(Tipo, levels = c("k=3 PC", "k=10 PC"))]

p_s5c <- ggplot(delta_dt, aes(x = factor(Scadenza), y = Delta_bp, fill = Tipo)) +
  geom_col(position = position_dodge(width = 0.75), width = 0.65) +
  geom_hline(yintercept = 0, colour = "grey30", linewidth = 0.4) +
  scale_fill_manual(values = c("k=3 PC" = "#e66101", "k=10 PC" = "#5e3c99")) +
  labs(
    title = "Errore di ricostruzione per scadenza (tenor)",
    x     = "Scadenza (anni)",
    y     = "Errore = ricostruita − originale (bp)",
    fill  = NULL
  ) +
  plot_theme

ggsave(file.path(output_dir, "sez5_errore_per_tenor.pdf"),
       p_s5c, width = 9, height = 5, device = cairo_pdf)
cat("Grafico C (errore per tenor) salvato.\n")
