...
# Pulizia artefatti di esecuzioni precedenti (riscrittura con rinomina file)
...
alpha <- as.numeric(phasing_alpha["2027"])   # 20% (primo anno di applicazione)
...
# --- Asset condivisi con la dispensa 03 (Sez. 1-2 identiche): figura motivazione
#     (curva ufficiale EIOPA dic 2025) e tabella dati di input. Codice/valori
#     identici nei due script -> file byte-coerenti in output/shared/.
...
# Figura illustrativa condivisa (Sez.2): fattori di sconto dalla curva ufficiale EIOPA di
# dicembre 2025 (method-agnostica). Codice identico in 01/03.
# NOTA: fig_spot_fwd_media (spot vs forward, Smith-Wilson) NON e' piu' generata qui: questa
# dispensa tratta il nuovo metodo (bootstrap), quindi la figura equivalente (Figura 2 del tex)
# usa ora la ricostruzione bootstrap stessa (vedi "fig_spot_fwd_bootstrap" piu' avanti, dopo il
# calcolo di resD). L'asset condiviso resta generato da R/03 per la dispensa 03 (Smith-Wilson).
{
  ...
  message("  [OK] output/shared/fig_fattori_sconto.pdf")
}
...
# --- Excel condiviso: curva bootstrap completa (continua e annua comp.), 1-100a --
if (have_openxlsx) {
