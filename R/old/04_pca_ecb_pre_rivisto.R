...
# ------------------------------------------------------------------
# Grafico 0: esempio illustrativo di una curva dei tassi (per la sez. 1.1)
# Un'unica curva (l'ultima disponibile) per mostrare cosa rappresentano
# gli assi: scadenza (tenor/maturity) in ascissa, tasso di interesse
# ricevuto investendo fino a quella scadenza in ordinata.
# ------------------------------------------------------------------
example_row <- curve_df[.N]
example_dt <- data.table(
  MAT_NUM = as.integer(sub("Y$", "", maturity_cols)),
  Rate    = as.numeric(example_row[, ..maturity_cols])
)

p0 <- ggplot(example_dt, aes(x = MAT_NUM, y = Rate)) +
...
ggsave(file.path(output_dir, "step0_esempio_curva_tassi.pdf"),
       p0, width = 8, height = 4.5, device = cairo_pdf)
...
# Etichette con la varianza cumulata per le prime k_show componenti
cum_lab_dt <- expl_dt[1:k_show]
cum_lab_dt[, Label := sprintf("%.1f%%", 100 * CumVar)]
...
p4 <- ggplot(expl_dt, aes(x = PC)) +
  ...
  geom_label(data = cum_lab_dt, aes(y = CumVar, label = Label),
...
# ------------------------------------------------------------------
# Scores mensili con media mobile a 6 mesi
# Il rumore mese su mese rende difficile leggere il trend (es. il rialzo
# del 2022) direttamente dalla serie grezza: la media mobile lo rende visibile.
# Un solo grafico (serie grezza + media mobile sovrapposte) sostituisce quello
# con la sola serie grezza.
# ------------------------------------------------------------------
scores_dt_m[, ScoreMA := data.table::frollmean(Score, n = 6, align = "right"),
            by = PC]

p6b <- ggplot(scores_dt_m, aes(x = TIME_PERIOD)) +
...
ggsave(file.path(output_dir, "step3_scores_mensili_ma_m.pdf"),
       p6b, width = 11, height = 9, device = cairo_pdf)
...
# ------------------------------------------------------------------
# Errore di ricostruzione per scadenza sui mesi selezionati
# La Figura 7 mostra le due curve sovrapposte: qui si scompone la
# differenza (ricostruita - osservata) scadenza per scadenza, per capire
# dove si concentra il residuo di ciascun mese.
# ------------------------------------------------------------------
err_tenor_dt <- rbindlist(lapply(seq_len(nrow(sel_months)), function(ii) {
  idx_i <- sel_months$IDX[ii]
  month_lab <- paste0(sel_months$Tipo[ii], ": ",
                      format(sel_months$YM[ii], "%b %Y"))
  osservata  <- as.numeric(Delta_R_m[idx_i, ])
  ricostruita <- as.numeric(X_hat_m[idx_i, ]) + mean_delta_m
  data.table(
    Mese = month_lab,
    MAT_NUM = as.integer(sub("Y$", "", maturity_cols)),
    Errore_bp = (ricostruita - osservata) * 100
  )
}))
err_tenor_dt[, Mese := factor(Mese, levels = unique(Mese))]

p7b <- ggplot(err_tenor_dt, aes(x = factor(MAT_NUM), y = Errore_bp)) +
...
ggsave(file.path(output_dir, "step3_errore_ricostruzione_tenor_m.pdf"),
       p7b, width = 9, height = 9, device = cairo_pdf)
...
# --- Curva EIOPA al 30/06/2026 (EUR, spot rate no VA) --------------
# Fonte: dati/eiopa_zips/EIOPA_RFR_20260630.zip
#        EIOPA_RFR_20260630_Term_Structures.xlsx, foglio RFR_spot_no_VA, col. "Euro"
y_new <- c(
  "1Y"  = 0.02592, "2Y"  = 0.02607, "3Y"  = 0.02612, "4Y"  = 0.02610,
  "5Y"  = 0.02628, "6Y"  = 0.02659, "7Y"  = 0.02701, "8Y"  = 0.02745,
  "9Y"  = 0.02784, "10Y" = 0.02850, "11Y" = 0.02870, "12Y" = 0.02911,
  "13Y" = 0.02952, "14Y" = 0.02985, "15Y" = 0.03033, "16Y" = 0.03068,
  "17Y" = 0.03087, "18Y" = 0.03095, "19Y" = 0.03098, "20Y" = 0.03098
)
...
delta_dt <- rbindlist(list(
  data.table(Scadenza = maturities_num,
             Delta_bp = (recon_list[["k=3"]]  - y_new) * 1e4,
             Tipo     = "k=3 PC"),
  data.table(Scadenza = maturities_num,
             Delta_bp = (recon_list[["k=10"]] - y_new) * 1e4,
             Tipo     = "k=10 PC")
))
...
# ==============================================================
# Tabella LaTeX — curva originale vs ricostruita (k=3) + delta per scadenza
# Frammento pronto per \input in dispense/04_pca_ecb.tex (Sezione 6).
# ==============================================================
tenor_labels_tex <- paste0(maturities_num, "Y")
orig_pct         <- as.numeric(y_new) * 100
ric_k3_pct       <- recon_list[["k=3"]] * 100
delta_k3_bp      <- (recon_list[["k=3"]] - y_new) * 1e4

tabella_k3_tex <- c(
  "\\begin{center}",
  "\\renewcommand{\\arraystretch}{1.25}",
  "\\resizebox{\\textwidth}{!}{%",
  paste0("\\begin{tabular}{l *{", length(maturities_num), "}{c}}"),
  "\\toprule",
  paste("Scadenza &", paste(tenor_labels_tex, collapse = " & "), "\\\\"),
  "\\midrule",
  paste("Originale (\\%) &",
        paste(sprintf("%.3f", orig_pct), collapse = " & "), "\\\\"),
  paste("Ricostruita $k=3$ (\\%) &",
        paste(sprintf("%.3f", ric_k3_pct), collapse = " & "), "\\\\"),
  paste("Delta (bp) &",
        paste(sprintf("%+.2f", delta_k3_bp), collapse = " & "), "\\\\"),
  "\\bottomrule",
  "\\end{tabular}}",
  "\\end{center}"
)
tabella_k3_file <- file.path(output_dir, "sez5_tabella_k3.tex")
writeLines(tabella_k3_tex, tabella_k3_file)
cat("Tabella k=3 (originale + ricostruzione + delta) salvata:", tabella_k3_file, "\n")
